...
const validateCreateMovieRequest = (req, res, next) => {
  ...
  if (!req.body.description) {
    return res.status(400).json(
      buildBadRequest("The description of the movie is not present in the request")
    );
  }
  ...
  if (!req.body.trailerUrl) {
    return res.status(400).json(
      buildBadRequest("The trailerUrl of the movie is not present in the request")
    );
  }
  ...
  if (!req.body.releaseDate) {
    return res.status(400).json(
      buildBadRequest("The releaseDate of the movie is not present in the request")
    );
  }
  ...
  if (!req.body.director) {
    return res.status(400).json(
      buildBadRequest("The director of the movie is not present in the request")
    );
  }
